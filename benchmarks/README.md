...
文件 2026-08-25-real-streaming-context-ttft.json 是当前线上服务在 2026-08-25 重新运行的结果：约 2.8K、5.6K、8.4K 实际输入，各连续串行 3 次。请求使用唯一合成英文上下文，避免 Prefix Cache 影响；输出固定 128 tokens，流式计时。
...
旧的 20K 合成压测不再作为首页代表速度；旧 P3/P4 或 2026-08-21 A/B 数字也不再进入速度基线，因为它们来自非流式请求脚本，TTFT 是按固定预填充速度反推的估算值，并非首个思考字符实测。
